...
class MosaicoDB:
    ...
    def adicionando_estacoes(self,uf,mapabr):
        ...
        add_lote_final = '''] AS dado CREATE  (n:EstacaoRadioBase { num_estacao:dado.num_estacao, fistel:dado.fistel, 
                                                   nome_entidade:dado.nome_entidade, UF:dado.UF, cod_ibge:dado.cod_ibge,
    ...
    def conectando_erb_antena(self, uf,mapabr):
        ...
        for erb in num_estacao:
            ...
            for id,info in estacao.iterrows():
                ...
                # add.append([cod_neo4j])
                # print(cod_neo4j)
                self.conn.executeDB(cod_neo4j,database=mapabr)
                i+=1
        # df = pd.DataFrame(add, columns=['Nome'])
        ...
        for estacao_erb in num_estacao:
            ...
            for id,info in estacao.iterrows():
                ...
                if(not pd.isna(info['tipoTecnologia'])):
                    consulta_conecta = f"""MATCH (ant{i}:Antena {{UID: \'{info['_id']}\' }}), (tec{i}:TecnologiaComunicacao {{ num_estacao: {estacao_erb}, tecnologia: \'{info['Tecnologia']}\',tipoTecnologia:\'{info['tipoTecnologia']}\' }}) CREATE (ant{i})-[:OPERA_SOB]->(tec{i});"""
                else:
                ...
                self.conn.executeDB(consulta_conecta,database=mapabr)
                i+=1
    ...
    def adicionando_equipamento_modularizacao(self, uf,mapabr):
        ...
        for estacao_erb in num_estacao:
            ...
            for info in filtrando_tecnologias:
                ...
                if(not pd.isna(info[0])):
                    autorizacao += f',codigo_equipamento: {int(info[0])}  '
    ...
    def conectando_antena_equipamento(self, uf, mapabr):
        ...
        for estacao_erb in num_estacao:
            ...
            for id,info in estacao.iterrows():
                ...
                self.conn.executeDB(relacionamento, database=mapabr)
                i+=1
        df = pd.DataFrame(add, columns=['Nome'])
...
def montando_estrutura(uf, mapabr):
    sistema = MosaicoDB(uf)
    # sistema.adicionando_antenas(uf, mapabr)
    # print('adicionando_antenas')
    # sistema.adicionando_estacoes(uf, mapabr)
    # print('sistema.adicionar_estacoes')
    # sistema.conectando_erb_antena(uf, mapabr)
    # print('sistema.conectando_erb_antena')
    # sistema.adicionando_tecnologias(uf, mapabr)
    # print('sistema.adicionando_tecnologias')
    # sistema.conectando_antena_tecnologia(uf, mapabr)
    # print('sistema.conectando_antena_tecnologia')
    # sistema.conectado_download_upload(uf, mapabr)
    # print('sistema.conectado_download_upload')
    # sistema.adicionando_infraestrutura(uf, mapabr)
    # print('sistema.adicionando_infraestrutura')
    # sistema.conectando_antena_infraestrutura(uf, mapabr)
    # print('sistema.conectando_antena_infraestrutura')
    sistema.adicionando_equipamento_modularizacao(uf, mapabr)
    print('sistema.adicionando_equipamento_modularizacao')
    # sistema.conectando_antena_equipamento(uf, mapabr)
    # print('sistema.conectando_antena_equipamento')

montando_estrutura('AC','sistemaintegrado')
print(F'montando_estrutura(\'AC\',\'sistemaintegrado\')')
montando_estrutura('RS','sistemaintegrado')
print(F'montando_estrutura(\'RS\',\'sistemaintegrado\')')
montando_estrutura('RJ','sistemaintegrado')
print(F'montando_estrutura(\'RJ\',\'sistemaintegrado\')')
montando_estrutura('GO','sistemaintegrado')
print(F'montando_estrutura(\'GO\',\'sistemaintegrado\')')
montando_estrutura('BA','sistemaintegrado')
print(F'montando_estrutura(\'BA\',\'sistemaintegrado\')')