...
class MapaIBGE:
    ...
    def adicionar_populacao_cidade(self, mapa):
        ...
        for id,dados_ibge in populacaomuninipios_2024.iterrows():
            print(f" MATCH (city:Cidade {{cod_ibge: {dados_ibge['COD_MUNICIPIO']} }}) SET city.populacao2024 = {dados_ibge['POPULACAO_ESTIMADA']} RETURN  city; ")
            self.conexao_db.executeDB(f" MATCH (city:Cidade {{cod_ibge: {dados_ibge['COD_MUNICIPIO']} }}) SET city.populacao2024 = {dados_ibge['POPULACAO_ESTIMADA']} RETURN  city; ", 
                                 mapa)
    ...
    def adicionar_populacao_estado(self, mapa):
        ...
        for id,dados_ibge in populacaoestados_2024.iterrows():
            self.conexao_db.executeDB(f" MATCH (city:Estado {{estado: \'{dados_ibge['UF']}\' }}) SET city.populacao2024 = {dados_ibge['POPULACAO_ESTIMADA']} RETURN  city; ", 
                                 mapa)
            
            
    def adicionar_codigo_nacional(self, mapabr):
        path = lambda uf: f'codigo_nacional/codigo_nacional_{uf}.csv'
        
        print("ok")
        for dados_ibge in DadosGeoespacialBR.estadosBRCSV():
            cod_nacional= pd.read_csv(path(dados_ibge[1]), sep=',',low_memory=False, on_bad_lines='warn')
            for id, dados in cod_nacional.iterrows():
                #Código Nacional,Código IBGE
                print(f""" MATCH (city:Cidade {{cod_ibge: {dados['Código IBGE']} }}) SET city.codigo_nacional = {dados['Código Nacional']} RETURN  city; """)
            
                self.conexao_db.executeDB(f""" MATCH (city:Cidade {{cod_ibge: {dados['Código IBGE']} }}) SET city.codigo_nacional = {dados['Código Nacional']} RETURN  city; """, mapabr)
    ...
    def montarDB(self, mapa='mapaibge'):
        ...
        self.adicionar_codigo_nacional(mapa)
        print('adicionar_codigo_nacional(self, mapa)')
